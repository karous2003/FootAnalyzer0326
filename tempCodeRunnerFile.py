...
FOOT_REMOVEBG_FOLDER = 'static/foot_removebg'
...
model_path = "kypoint/kypoint/model/model/pytorch-keypoint-r-cnn/2025-02-09_22-25-32_Newdata/labelme-keypoint-eyes-noses-dataset-keypointrcnn_resnet50_fpn.onnx"
...
def process_insole(image_path):
    ...
    在最終調整尺寸後的圖像上測量並繪製鞋墊總長、前掌寬、中足寬與後跟寬等尺寸。
    ...
    # 二值化梯度圖
    _, binary = cv2.threshold(gradient_magnitude, 5, 205, cv2.THRESH_BINARY)
    ...
    refined_binary = cv2.bitwise_and(binary, binary, mask=cv2.bitwise_not(shadow_mask))
    
    # 形態學閉運算填補小孔
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
    refined_binary = cv2.morphologyEx(refined_binary, cv2.MORPH_CLOSE, kernel)
    ...
    if topmost[1] < bottommost[1]:
        ...
        rotated_binary = rotate_image(refined_binary, -angle_degrees)  # 使用 refined_binary
    elif topmost[1] > bottommost[1]:
        angle_degrees = 180 - calculate_rotation_angle(topmost, bottommost)
        rotated_image = rotate_image(segmented_image, angle_degrees)
        rotated_binary = rotate_image(refined_binary, angle_degrees)
    else:
        rotated_image = segmented_image
        rotated_binary = refined_binary
    ...
    y_mid = (rotated_max_contour[:, :, 1].min() + rotated_max_contour[:, :, 1].max()) // 2
    ...
    final_image = cv2.resize(rotated_image, (2480, 3508), interpolation=cv2.INTER_AREA)
    final_binary = cv2.resize(rotated_binary, (2480, 3508), interpolation=cv2.INTER_NEAREST)
    
    # 在最終圖像上重新檢測輪廓並測量
    final_contours, _ = cv2.findContours(final_binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not final_contours:
        raise Exception("最終圖像中未找到輪廓")
    final_max_contour = max(final_contours, key=cv2.contourArea)
    
    # 計算鞋墊長度
    bottom_point = tuple(final_max_contour[final_max_contour[:, :, 1].argmax()][0])
    front_point = tuple(final_max_contour[final_max_contour[:, :, 1].argmin()][0])
    foot_length_pixels = math.dist(bottom_point, front_point)
    
    # 利用 A4 紙比例估算每公分的像素數
    a4_width_cm = 21.0
    a4_length_cm = 29.7
    pixels_per_cm = ((2480 / a4_width_cm) + (3508 / a4_length_cm)) / 2
    ...
    # 繪製鞋墊長（藍線）
    draw_line(final_image, bottom_point, front_point, (255, 135, 0))
    cv2.circle(final_image, bottom_point, 12, (255, 135, 0), -1)
    cv2.circle(final_image, front_point, 12, (255, 135, 0), -1)
    
    # 根據方向調整計算參數
    if orientation == "normal":
        y_threshold = front_point[1] + int(foot_length_pixels * 0.5)
        front_half_points = [pt[0] for pt in final_max_contour if pt[0][1] <= y_threshold]
    ...
    if front_half_points:
        ...
        draw_line(final_image, left_most, right_most, (0, 0, 255))
        cv2.circle(final_image, left_most, 12, (0, 0, 255), -1)
        cv2.circle(final_image, right_most, 12, (0, 0, 255), -1)
        forefoot_center = ((left_most[0] + right_most[0]) // 2, (left_most[1] + right_most[1]) // 2)
        cv2.circle(final_image, forefoot_center, 12, (0, 0, 255), -1)
    ...
    left_point = find_nearest_contour_point(final_image, midfoot_pt, "left", final_max_contour, midfoot_pt[1])
    right_point = find_nearest_contour_point(final_image, midfoot_pt, "right", final_max_contour, midfoot_pt[1])
    midfoot_width = 0
    if left_point and right_point:
        midfoot_width = math.dist(left_point, right_point) / pixels_per_cm
        draw_line(final_image, left_point, right_point, (255, 0, 155))
        cv2.circle(final_image, left_point, 12, (255, 0, 155), -1)
        cv2.circle(final_image, right_point, 12, (255, 0, 155), -1)
    
    # 後跟寬
    heel_center = (bottom_point[0], heel_y)
    heel_width_left = find_nearest_contour_point(final_image, heel_center, "left", final_max_contour, heel_y)
    heel_width_right = find_nearest_contour_point(final_image, heel_center, "right", final_max_contour, heel_y)
    heel_width = 0
    if heel_width_left and heel_width_right and forefoot_center:
        ...
        draw_line(final_image, heel_width_left, heel_width_right, (0, 255, 255))
        cv2.circle(final_image, heel_width_left, 12, (0, 255, 255), -1)
        cv2.circle(final_image, heel_width_right, 12, (0, 255, 255), -1)
        ...
        while True:
            ...
            if cv2.pointPolygonTest(final_max_contour, current_end, True) < 0:
                green_line_endpoint = current_end
                break
        if green_line_endpoint is not None:
            cv2.circle(final_image, green_line_endpoint, 12, (0, 255, 0), -1)
            cv2.circle(final_image, green_line_start, 12, (0, 255, 255), -1)
            cv2.line(final_image, green_line_start, green_line_endpoint, (0, 255, 0), 10)
    else:
        print("無法取得繪製中心線的必要點")
    ...
    line_spacing = 80
    measurements = [
        {"text": f"Insole Length: {insole_length_cm:.2f} cm", "color": (255, 135, 0)},
        {"text": f"Forefoot Width: {forefoot_width:.2f} cm", "color": (0, 0, 255)},
        {"text": f"Midfoot Width: {midfoot_width:.2f} cm", "color": (255, 0, 155)},
        {"text": f"Heel Width: {heel_width:.2f} cm", "color": (0, 255, 255)}
    ]
    for i, measurement in enumerate(measurements):
        text = measurement["text"]
        text_color = measurement["color"]
        y_pos = start_y + i * line_spacing
        cv2.putText(final_image, text, (start_x, y_pos), font, font_scale, text_color, thickness)
...
def foot_removebg(image_path):
    ...
    full_image_url = os.path.join(OUTPUT_FOLDER, f"{basename}.png")
    transparent_full.save(full_image_url, format="PNG")
...
def draw_keypoints(test_img, keypoints, left_hva, right_hva):
    ...
    if left_hva is not None:
        cv2.putText(image_cv, f"Left HVA: {left_hva:.2f}", (10, 30), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    if right_hva is not None:
        cv2.putText(image_cv, f"Right HVA: {right_hva:.2f}", (10, 60), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            ...
            if stage == 'removebg':
                ...
                for i, mask in enumerate(masks):
                    if boxes[i].conf.item() <= 0.95:
                        return jsonify({"error": "去背結果不佳，請重新拍照", "success": False}), 400
                ...
                background_removed_path = os.path.join(HALLUX_OUTPUT_FOLDER, background_removed_filename)
                transparent.save(background_removed_path, format="PNG")
                image_url = url_for('get_hallux_file', filename=background_removed_filename, _external=True)
